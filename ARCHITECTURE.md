...
- **It's an SPA architecturally, without an SPA framework.** The web app is a single `index.html` mount point (`#app`), all UI built programmatically via `document.createElement`, hash-based navigation (`#/`, `#/review`), and shared in-memory state (auth session, FX cache, currency symbols, edit mode) — no full-page reloads. What it deliberately lacks is an SPA *framework*: no React/Vue, plain ES modules and small render functions instead. Introducing a framework now would be solving a problem the app doesn't have. Keep the thin `/api` layer so every Supabase query lives in one place — if the schema changes, there's one file per query type to update, not a grep through view code. If UI complexity outgrows this (once charts/budgets/PWA land per §13), the honest next step is a lightweight reactive layer (Preact or Alpine.js) rather than jumping straight to React.
...
- **`utils/fx.js` isolates the still-open FX provider choice (§15).** Whichever API gets picked, it's a single function with one signature (`convert(amount, from, to) → number`) and its own caching — nothing else in `/web` should know which provider it is. Since ADR 0003, `convert` also takes an optional date and freezes the rate at the transaction's own date when provided (historical lookup), so past rows don't drift as live rates move.
