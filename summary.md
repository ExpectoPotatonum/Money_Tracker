...
2. **More parser templates / more apps** — TnG, CIMB, Wise, and HLB Connect are live. Wise uses amount-first format (`25.90 MYR spent at Apple`), which the loose fallback missed; a Wise v1 template was added (migration `202609080001`). HLB Connect (`my.com.hongleongconnect.mobileconnect`) was added to the Android capture allow-list — its `parser_templates` still need building from real samples, and an HLB card spend may now notify twice (HLB app + Samsung Wallet), expect the rare duplicate to delete by hand. Adding more banks/wallets still needs real captured samples first (AGENTS.md §14 rollout).
