-- 025: reject marketing/reward notifications before parsing (parser_templates.
-- reject_pattern)
--
-- WHY: TnG sends reward/marketing pushes alongside real transaction ones. The
--   ones without a decimal amount already fail cleanly ("no extractable
--   amount"), but any that CARRY a currency amount with decimals would slip
--   through the loose fallback as a phantom `needs_review` transaction with
--   NULL merchant/category — the exact recurring symptom. This adds a
--   data-driven guard: if a notification body matches an active template's
--   reject_pattern, it is marked `ignored` (deliberately not a transaction,
--   §9) and never parsed.
--
-- Scope: reject_pattern is per-template, so each app controls its own keyword
--   set. Seeded for TnG only. Verified: the keyword set rejects points/
--   cashback/voucher/balance-low/reload pushes and rejects ZERO real TnG
--   transaction bodies (paid/transferred/deducted/received all stay live).
--
-- Idempotent (create or replace / NOT EXISTS / resync safe to re-run).

-- 1. Column (no GRANT needed — existing column on a reference table the parser
--    reads; authenticated never writes parser_templates).
alter table parser_templates
  add column if not exists reject_pattern text;

-- 2. Redefine the parser to reject before matching a template.
...
-- 3. Seed the reject pattern for every active TnG template.
update parser_templates
  set reject_pattern =
      'points|cashback|voucher|reward|earned|redeem|balance|reload|scan & pay'
  where package_name = 'my.com.tngdigital.ewallet'
    and active
    and (reject_pattern is null or reject_pattern = '');

-- 4. Re-parse: converts the reward pushes to 'ignored' and picks up the v6
--    "RM X has been successfully transferred to <M>." format.
