...
components/
  ...
  nlModal.js         "Add from text" dialog — sentence → AI preview → manual insert; optional click-to-talk voice input (Web Speech API)
...
utils/
  ...
  sources.js         source (payment-method) picker — presets + Custom text, shared by NL modal & edit mode
views/
...
- MYR conversion happens in the web UI at display time, freezing the rate at each transaction's own date (ADR 0003); the database only ever stores original amount + currency.
...
- FX conversion for the MYR dashboard total uses the **historical rate at each transaction's own date** (ADR 0003), not a live latest rate — so a past transaction's MYR value doesn't drift as a rate moves between page loads. Still fetched via the FX API, not manual entry or a static table.
...
- **FX provider: Frankfurter** (`utils/fx.js`, `https://api.frankfurter.dev/v2`) — fetched once per (pair, date) — historical at the transaction's date (ADR 0003), latest when no date is given — and cached in memory for the page load; returns `null` (never throws) so an unavailable rate degrades to showing the original amount (ADR 0001). Live hosts are in `web/ARCHITECTURE.md`.
- **Manual edit/delete mode** on the dashboard (web) — the owner corrects anything parsing gets wrong (e.g. an e-wallet and its underlying bank card both notify the same real transfer; no automated cross-app dedup — one is deleted by hand in edit mode instead). Inline per-cell editing behind an **Edit mode** toggle: date (MYT `datetime-local`), receiver raw merchant text, category dropdown, amount + currency + direction, and a **comment/notes** field. The Receiver field edits **only** `merchant_raw` — `merchant_display` is not surfaced as an edit control (the owner prefers to see the raw text; the display name comes from `merchant_rules` resolution server-side). **No per-row Save** — edits are accumulated per row and PATCHed in one batch when Edit mode is toggled **off**, and are also flushed on navigation so they're never silently dropped. **Delete** (DELETE, gated behind a confirm) stays the only per-row action, shown as a red-cross icon. Table columns read **Receiver** (was Merchant) and **Sent from** (was Source). `authenticated` already has UPDATE/DELETE on `transactions` under the owner_only RLS policy (migration 202608180001), so this is web-only, no new grant. Dates render pinned to `Asia/Kuala_Lumpur`.
...
- **Reward/marketing pushes are rejected, not parsed** (§9 `reject_pattern`) — TnG `points|cashback|voucher|reward|earned|redeem|balance|reload|scan & pay` rows become `ignored`. Real TnG transaction bodies (paid/transferred/deducted/received) verified to contain none of these keywords. Confirmed by the owner (2026-09-02): genuine TnG cashback stays `ignored` — it is marketing noise, deliberately not treated as income.
...
- **Keyword auto-categorization** (migration 202609060001/002) — `merchant_rules` gains **category-only** regex fallbacks: Food & Dining, Health & Wellness, Transport, and Shopping each get one `regex` rule (priority 5) that sets a category without renaming the merchant. Named-merchant rules (priority 20: starbucks→Food & Dining, aeon/muji→Shopping, HOO JET YUNG→Finance & Transfer) always win over keyword rules, e.g. "AEON FOOD FAIR" parses Shopping, not Food & Dining. Requires `merchant_rules.normalized_name` to be **nullable** (202609060001 alters it): `NULL` now means "category-only rule", while the parser's existing `coalesce(raw, normalized_name)` keeps keeping the raw text as display. `apply_category_hints()` backfills existing `category_id IS NULL` transactions with the best matching rule — same resolution semantics as `parse_raw_notification`, and it **never overwrites a row that already has a category** (manual picks survive). Adding a keyword = edit the category's regex rule + `select apply_category_hints();` (rule edits here are allowed — the immutability trigger only guards `parser_templates`).
- **Categories diverge from the seed file in the live project.** 202608160001 seeds `Health & Fitness` + `Transfers`; live instead has **`Health & Wellness`** (added by 202609060002 — there was no health category at all) and a distinct **`Finance & Transfer`** alongside `Transfers`. Keyword rules resolve category **by name** at insert time, so they bind to whatever actually exists — that's why the health rule initially got `category_id = NULL` (no matching category row) and matched nothing until 202609060002 created the category and backfilled. If you're ever staring at rules that exist but categorize nothing, check `mr.category_id` before suspecting the regex.
- **Inline category picker** (web, 2026-09-06) — read-only rows with no category show a **"— Pick…" dropdown** in the Category cell instead of the em-dash; choosing one PATCHes immediately (`updateTransaction` + refresh), the same pattern as the 🔁 recurring toggle. Edit mode's full category dropdown is unchanged and still covers re-categorizing rows that already have one (the picker never appears on those). Covered by `web/tests/dashboard.spec.js`.
  - **Selectable Source (payment method)** (web, 2026-09-06) — NL modal preview adds a **Source** field and edit mode makes the **Sent from** column editable, both via `buildSourceEditor` (`utils/sources.js`): presets `Cash / Bank / e-Wallet / Manual / Card` plus a **Custom…** text input for anything else (`GrabPay`, etc.). Non-preset values (e.g. a real notification's app label `TnG eWallet`) render as Custom with the label pre-filled. Only `source_app_label` is written — `source_package` stays `'manual'` for NL rows and the Android package name for notification rows, and the `status`/confidence semantics are untouched. Covered by `web/tests/ai.spec.js` + `web/tests/dashboard.spec.js`.
- **Voice capture** (web, 2026-09-08) — the NL "Add from text" modal gains a click-to-talk 🎤 button (`hookUpVoice` in `components/nlModal.js`) wired to the Web Speech API; it transcribes into the textarea (editable) before the normal AI parse. Button is hidden where `SpeechRecognition` is unavailable. Recording/denied/error/unsupported messages are i18n'ed (en/zh). Recognizer uses `en-US` (Chrome's `en-MY` is often unmapped and gibberish) with `interimResults` + `continuous`, and renders final+interim segments **by replacement**, never **append** — the original appending version doubled the transcript on every event ("Paid Paid Paid…"). Phase F's web half: full on-phone voice via Android `SpeechRecognizer` is still the deferred app side. Tested by `web/tests/ai.spec.js` (mocked recognizer that replays interim→final events).
- **Notification text timestamps are LOCAL MYT** (migration 202609070001) — `parse_raw_notification` now parses a `date_format` txn timestamp as `(to_timestamp(…) AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Kuala_Lumpur'` instead of a bare `to_timestamp(…)`. A CIMB v1 "… on 07-Sep-2026, 11:03:36" is 11:03 *MYT* = 03:03 UTC; before the fix it was stored as 11:03 UTC and rendered 19:03 on the KL-pinned dashboard. (The first attempt — a single `AT TIME ZONE 'Asia/Kuala_Lumpur'` — was wrong the *other* way: on this PG `to_timestamp(text, text)` returns `timestamptz` assuming the session zone (= UTC), so `11:03+00 AT TIME ZONE KL` yields a naive `19:03`, stored as `19:03+00`. The double form first reads the UTC wall-clock as naive, then reinterprets that as MYT: `11:03+00 → naive 11:03 → 03:03+00`.) Affects only templates whose `date_format` carries a time-of-day with no timezone (today: just CIMB v1; TnG/Samsung have no txn_date and fall back to `posted_at`, already correct UTC). The migration also repairs already-parsed CIMB rows **in place** (re-derives txn time from the raw text with the same double form), deliberately avoiding a re-parse so manual category/merchant edits on those rows survive. Any future template with a clock time in `date_format` must assume the text is local MYT.
- **Dark mode + PWA (web, Phase 2 of import-candidate-features.md, 2026-09-19)** — owner decision: **pure-black OLED (`#000`)**. Dark mode: `mt_theme` setting (`system|dark|light`, default `system`) read/written via `lib/settings.js`, `lib/theme.js` applies `data-bs-theme` (Bootstrap 5.3) on `<html>` plus a `theme-color` meta, tracks `prefers-color-scheme` changes live while in `system` mode, and `cycleTheme` walks `system → dark → light` (the wheel starts from `system` so the *first* click from the default is meaningful — the original `light → dark → system` order meant click 1 landed on light again, caught by e2e). `src/style.css` re-pins the dark palette: `--bs-body-bg:#000`, near-black `--bs-secondary-bg` (cards/dropdowns/modals) + table stripes, and a `.navbar.bg-light` override so the nav doesn't glare. Nav button shows the **resolved** icon (🌙/☀️). **PWA**: `vite-plugin-pwa` 1.3.0 (Workbox) in `vite.config.js` — `registerType:'autoUpdate'`, `navigateFallback:'/index.html'`, manifest (`start_url '/'`, standalone, `#000` theme_color, `public/icon.svg`), precache globs, and a NetworkFirst runtime cache `supabase-reads` for `/rest/v1/*` GETs (30 entries / 7 days, 3s timeout) so the last-loaded RLS-scoped data is viewable offline. `netlify.toml` no-cache headers for `sw.js`/`workbox-*.js`/`manifest.webmanifest` (a CDN-cached `sw.js` pins the old app forever). e2e blocks service workers (`serviceWorkers:'block'`) so the built SW can't intercept the route mocks; dev emits no SW at all (plugin is build-only).
- **Android quick-add + settings UI (Phase 5, 2026-09-25)** — the long-deferred "full on-phone voice" half of Phase F. Three-tab `MainScreen` (Status / Quick Add / Settings). **Quick Add** = `SpeechRecognizer` voice (🎤, replace-don't-append transcription, `zh-CN` when device locale is zh else `en-US` — mirrors the web) OR typed NL → Gemini (`parse/` — LlmClient + NlPrompts + JsonExtractor + TransactionNormalizer) → editable preview → `insertTransaction` to Supabase keyed on id (same manual-row contract as the web: `source_package='manual'`, `confidence='low'`, source picker presets Cash/Bank/e-Wallet/Manual/Card + Custom from `utils/sources.js` mirror); categories/accounts GET populate pickers; stale-token clear-and-retry-once on 401/403. **Settings** carries credentials + LLM key/model/enable (moved out of `MainViewModel` into `SettingsViewModel`) plus listener/battery/package status. `SupabaseApi` gained `listCategories`/`listAccounts` (RLS-scoped GET helper mapping 401/403 → `UnauthorizedException`) and `insertTransaction`. Voice language rule: mirror web `zh-CN`/`en-US`.
- **Recurring auto-entry (Phase 6, 2026-09-25)** — the phone owns the clock (Free tier: no `pg_cron`/scheduled Edge Functions). Migration `202609250001_recurring_entries.sql` adds owner-scoped `recurring_entries` (name, amount, currency, direction, category, **account**, frequency daily/weekly/monthly/yearly, `interval_step`, `day_of_month` clamp, `next_due_date`, active; GRANT + RLS per §17) plus `transactions.recurring_entry_id` (FK `on delete set null` — unlinks, never deletes, history) + `recurring_due_date` under the partial unique index `transactions_recurring_uq (recurring_entry_id, recurring_due_date) where recurring_entry_id is not null`. `RecurringCheckWorker` (HiltWorker) runs as a 24h periodic + a one-off on every app wake/capture: reads active cycles with `next_due_date <= today(MYT)`, materializes each as a manual `is_recurring` row dated **00:00 MYT on its due date** (KL-pinned like the dashboard), using `Prefer: resolution=ignore-duplicates` on the dedup pair so retries can't double-record, and only then PATCHes `next_due_date` forward (one interval from the due date — **no backfill**, a lagged catch-up materializes one missed cycle per pass). Pure math lives in `RecurringMaterializer` (unit-tested: `advance` incl. month-end clamp, MYT instants, row shape); web management path = `api/recurring.js` (`listRecurringEntries`/`saveRecurringEntry`/`deleteRecurringEntry`); a manager **UI** is the natural follow-up — entries can be created in the SQL Editor meanwhile.
...
- **More parser templates / more apps** — TnG, CIMB, Wise, and the HLB Connect app are live. Wise debit card payments (`25.90 MYR spent at Apple`) use amount-first format (`AMOUNT CUR`), which the loose fallback didn't match (it only catches `CUR AMOUNT`). A Wise v1 template was added (migration `202609080001`). The Hong Leong Bank app (`my.com.hongleongconnect.mobileconnect`) was added to the Android capture allow-list (TargetPackages.kt) so the tracker now captures HLB Connect notifications directly — not just HLB card debits surfaced through Samsung Wallet; its `parser_templates` still need to be built from real captured samples (§14), and an HLB card spend may now notify twice (HLB app + Samsung Wallet), so expect a rare duplicate to delete by hand in edit mode per §15. Adding more banks/wallets still needs real captured samples first (§14 rollout), which is the next real-world gap.
