...
- **Voice capture** (web, 2026-09-08) — the NL "Add from text" modal gains a click-to-talk 🎤 button (`hookUpVoice` in `components/nlModal.js`) wired to the Web Speech API; it transcribes into the textarea (editable) before the normal AI parse. Button is hidden where `SpeechRecognition` is unavailable. Recording/denied/error/unsupported messages are i18n'ed (en/zh). Phase F's web half: full on-phone voice via Android `SpeechRecognizer` is still the deferred app side. Tested by `web/tests/ai.spec.js` (mocked recognizer).
