...
- **More parser templates / more apps** — TnG, CIMB, and Wise are live. Wise debit card payments (`25.90 MYR spent at Apple`) use amount-first format (`AMOUNT CUR`), which the loose fallback didn't match (it only catches `CUR AMOUNT`). A Wise v1 template was added (migration `202609080001`). Adding more banks/wallets still needs real captured samples first (§14 rollout), which is the next real-world gap.
